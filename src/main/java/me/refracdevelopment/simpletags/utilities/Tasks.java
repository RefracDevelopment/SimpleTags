...
public class Tasks {
    ...
    public static void executeAtEntity(Entity entity, Runnable runnable) {
        ...
        SimpleTags.getInstance().getFoliaLib().getImpl().runAtEntity(entity, consumer);
    }

    public static void runAtEntityDelayed(Entity entity, Runnable runnable, long delay) {
        Consumer<WrappedTask> consumer = task -> runnable.run();
        SimpleTags.getInstance().getFoliaLib().getImpl().runAtEntityLater(entity, consumer, runnable, delay);
    }

    public static void runAsync(Runnable runnable) {
        Consumer<WrappedTask> consumer = task -> runnable.run();
        SimpleTags.getInstance().getFoliaLib().getImpl().runAsync(consumer);
    }

    public static void runAsyncTimer(Runnable runnable, long delay, long period, TimeUnit unit) {
        Consumer<WrappedTask> consumer = task -> runnable.run();
        SimpleTags.getInstance().getFoliaLib().getImpl().runTimerAsync(consumer, delay, period, unit);
    }
}